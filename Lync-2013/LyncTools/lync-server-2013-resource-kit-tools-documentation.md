...
Les répertoires de fichiers suivants peuvent être spécifiés par l’utilisateur, comme indiqué.
  ...
  - **Emplacement des fichiers journaux du serveur** Emplacement du dossier dans lequel les journaux du serveur de stratégie de bande passante sont stockés. Il s’agit généralement de \<fileserver\> \\ \<choice of FE\> \\ AppServerFiles \\ PDP.
...
Toutes les options s’excluent mutuellement. La syntaxe suivante est appliquée à la ligne de commande :
  ...
  - **-r \<n\>** paramètre : affiche la liste des \<n\> derniers appels en stationnement. Les informations suivantes sont affichées :
  ...
  - **-t \<n\>** tests de paramètre réservant une orbite dans la base de données afin d’afficher la randomisation des numéros d’orbites attribués.
...
Test de réservation orbite : les \<n\> tests de paramètre – t servent à réapprovisionner une orbite dans la base de données, comme illustré
...
![Options de ligne de commande pour les Dbanalyze.exe.](images/JJ945604.22bf3432-af6d-495b-8f48-d94c5d259523(OCS.15).jpg "Options de ligne de commande pour les Dbanalyze.exe.")
...
L’exécution d' LookupUserConsole.exe entraîne l’ouverture d’une invite de commandes qui accepte les adresses SIP et tente d’afficher des informations de routage du serveur Lync internes associées. Tapez **exit** pour quitter l’outil LookupUserConsole.
...
Voici certains des principaux scénarios d’utilisation de cet outil :
  ...
  - Michel est un nouvel employé de contoso et il rejoint une nouvelle équipe sur laquelle tous les membres sont configurés pour l’appel d’équipe, lorsque l’utilisateur a activé Microsoft Lync, l’administrateur est en mesure de définir les paramètres de son groupe d’appel d’équipe pour inclure tous les membres de son équipe, l’administrateur peut également ajouter Michel en tant que membre du groupe d’appel d’équipe.
...
Pour exécuter le script de SYSPrep.ps1 et installer les composants requis de Lync Server 2013, exécutez la commande suivante à partir d’une invite de commandes avec élévation de privilèges :
...
![Paramètres de déplacement-CsAnnouncementConfiguration.](images/JJ945604.7ab66ad3-d0db-4d77-8b93-ebccf0cb0663(OCS.15).jpg "Paramètres Move-CsAnnouncementConfiguration.")
