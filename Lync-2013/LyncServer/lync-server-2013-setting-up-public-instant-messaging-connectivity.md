...
1.  Lorsque le modèle requis est disponible pour l’autorité de certification, utilisez la cmdlet Windows PowerShell suivante à partir du serveur de périphérie pour demander le certificat.
    ...
    Le nom du certificat par défaut du modèle utilisé pour Lync Server est le serveur Web. Spécifiez uniquement \<template name\> si vous devez utiliser un modèle différent du modèle par défaut.
