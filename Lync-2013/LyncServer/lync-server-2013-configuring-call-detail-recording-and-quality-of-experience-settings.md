...
Après avoir associé un magasin de surveillance avec un pool frontal, configurez le magasin de surveillance, puis installez et configurez SQL Server Reporting Services et la surveillance des rapports pour gérer l’enregistrement des détails des appels (CDR) et la qualité d’utilisation (QoE) à l’aide de Lync Server Management Shell. Les applets de contrôle Lync Server Management Shell vous permettent d’activer et de désactiver la surveillance des CDR et/ou QoE pour un site particulier ou pour votre déploiement complet de Lync Server. pour ce faire, vous pouvez utiliser une commande aussi simple que celle-ci :
