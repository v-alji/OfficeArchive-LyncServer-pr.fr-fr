...
Avant d’activer la topologie après le déploiement de celle-ci, vérifiez que les enregistrements DNS et Active Directory suivants sont créés (à mesure que vous avez besoin de fonctionnalités spécifiques dictées) :
  ...
  - Un enregistrement SRV DNS existe pour chaque domaine SIP si vous envisagez d’utiliser l’ouverture de session automatique pour les clients sous la forme d’une \_ sipinternaltls \_ TCP. \<SIP domain\> Si vous allez utiliser la configuration manuelle pour les clients, cet enregistrement n’est pas nécessaire.
