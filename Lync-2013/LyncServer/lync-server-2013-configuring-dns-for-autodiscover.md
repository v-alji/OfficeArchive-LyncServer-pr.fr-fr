...
Les enregistrements DNS peuvent être des enregistrements CNAMe ou CNAMe, en fonction de votre capacité à créer des certificats avec le nouveau nom d’objet. Si vous n’êtes pas en mesure de demander et de déployer un nouveau certificat externe (public) avec le lyncdiscover.\<domain name\> Utilisez la procédure d’utilisation de la 80 port TCP/TCP. Les procédures suivantes décrivent la création d’enregistrements DNS internes et externes.
